...
sys.stdout = Logger("F:\keshan\keshab/log_object_result.txt")  # 保存屏幕输出到F盘


#test_path为对象图片，val_path为验证图片
test_path='F:\keshan\keshab/object.png'
val_path="F:\keshan\keshab/min2.png"
...
#类别信息
#循环对象
object_classes=list(range(0,16366))
...
for object_class in tqdm(object_classes):
    ...
    if add_number>0 and int(add_value/add_number)>6000:
        #循环需要改变的对象位置
        #对象为边界赋值为9900，否则不改变值
        print("%d ,是真值 " % (object_class))
        print("其值为：%d" % (int(add_value/add_number)))
        for location in range(len(locations1)):
            test_img.putpixel((locations1[location], locations2[location]),9900)
    #else:print('wutu')

test_img.save('F:\keshan\keshab/result/object_result3.png')
